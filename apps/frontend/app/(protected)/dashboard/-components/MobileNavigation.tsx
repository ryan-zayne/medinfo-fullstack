...
function MobileNavigation(props: MobileNavProps) {
	...
	return (
		...
		>
			...
			<nav className="mt-8 flex flex-col justify-start gap-5 font-medium text-nowrap lg:text-[22px]">
				{menuItems.map(({ href, icon, title }) => (
					<NavLink key={href} transitionType="navbar" href={href} className="flex items-center gap-4">
						{icon}
						{title}
					</NavLink>
				))}
				<NavLink transitionType="navbar" href="/" className="mt-8 flex items-center gap-4">
