...
export const redisQueueClient = new Redis(queueRedisURL, {
	...
	retryStrategy: (times: number) => {
		...
		consola.warn(`Redis reconnecting... attempt ${times}, delay ${delay}ms`);
		return delay;
	},
});

redisQueueClient.on("connect", () => {
	consola.info("Connected to Redis Queue Client!");
});

redisQueueClient.on("error", (error: NodeJS.ErrnoException) => {
	// == Don't log ECONNRESET as error - it's handled by reconnect
	if (error.code === "ECONNRESET" || error.message.includes("ECONNRESET")) {
		consola.warn(`Redis Queue Client ECONNRESET - will auto-reconnect`);
		return;
	}

	consola.error(`Redis Queue Client Error: ${error.message}`, error);
});
